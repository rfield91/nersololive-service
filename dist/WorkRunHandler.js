...
const workRunHandler = async () => {
    ...
    classes = Object.fromEntries(Object.entries(classes).filter(([key, runWork]) => runWork.run != -1 && runWork.work != -1));
    ...
    uploadRunWork(resultsUploader, process.env.RUN_WORK_RESULTS_UPLOAD_NAME, runWorkData);
};
const uploadRunWork = async (resultsUploader, uploadName, runWorkData) => {
    const uploadSuccess = await resultsUploader.upload(uploadName, JSON.stringify(runWorkData));
    if (!uploadSuccess)
        consoleErrorBeep();
};
const getRowData = (matches) => {
    const rowData = [];
...
const getClassList = (classString) => {
...
//# sourceMappingURL=WorkRunHandler.js.map