    ...
    parse() {
        ...
        tableRows.forEach((row) => {
            ...
            if (entryData != null && entryData.length == 18) {
                ...
                const classResult = new Result(car, carClassGroup, carClass, color, name, number, position, paxPosition, new RunInfo(parseFloat(rowData[16].text.trim()), parseFloat(rowData[6].text.trim())));
                let cleanCount = 0;
                let totalCones = 0;
                let dnfCount = 0;
                for (var i = 8; i <= 15; i++) {
...
//# sourceMappingURL=ResultsHTMLParser.js.map